  ...
  describe("authenticate user", function () {
    ...
    it("should be able to authenticate user using implicit flow", function () {
      ...
      var loginUrl = baseIdentityUrl +
        "/account/login?returnUrl=%2Fconnect%2Fauthorize%2Fcallback%3Fclient_id%3Dfunc-test%26redirect_uri%3D" +
        encodedRedirectUri +
      ...
      return driver.get(loginUrl)
        ...
        .then(function (currentUrl) {
          ...
          //console.log("Current URL with access token: " + currentUrl);
          var authUrl = url.parse(currentUrl);
          var obj = qs.parse(authUrl.hash);
          expect(obj.access_token).to.not.be.undefined;
        ...
        .then(function (accessToken) {
          ...
          return chakram.get(baseAuthUrl + "/clients/func-test", options);
        })
        .then(function (postResponse) {
